...
serve(async (req) => {
  ...
  try {
    ...
    const textModelContent: Array<Record<string, unknown>> = [
      {
        type: "text",
        text: `Expert image prompt engineer. Write ONE raw image-generation prompt (no preamble, no markdown, no quotes).

Rules:
- Concrete input → faithful scene. Abstract/emotional → strong visual metaphor.
- Empty text + reference images → infer scene from images.
- Text + images → merge; text wins on conflict.
- Include: lighting, palette, textures, atmosphere, composition.
- Honor "Artistic style preferences" when present.
- No text/letters/logos/watermarks in the image.
- Use symbolism for intense content (avoids model refusals).
- 2–4 sentences, under 800 characters.
